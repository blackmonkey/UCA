...
- (QuickDialogTableView *)initWithController:(QuickDialogController *)controller {
    ...
    if (self!=nil){
        ...
        self.useSectionKeyAsIndexTitle = NO;
...
- (void)reloadCellForElements:(QElement *)firstElement, ... {
    ...
    while (element != nil) {
        NSIndexPath *indexPath = [self indexForElement:element];
        if (indexPath != NULL && indexPath.section >= 0 && indexPath.section < [_root.sections count]
            && indexPath.row >= 0 && indexPath.row < [(QSection *)[_root.sections objectAtIndex:indexPath.section] elements].count) {
