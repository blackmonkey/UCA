...
-(UIToolbar *)createActionBar {
    UIToolbar *actionBar = [super createActionBar];
    NSMutableArray *items = [[NSMutableArray alloc] initWithArray:actionBar.items];

    UIBarButtonItem *completeButton = [[UIBarButtonItem alloc] initWithTitle:I18nString(@"补全")
                                                                       style:UIBarButtonItemStyleDone target:self
                                                                      action:@selector(handleActionBarComplete:)];
    [items insertObject:completeButton atIndex:(items.count - 1)];
...
- (BOOL)handleActionBarComplete:(UIBarButtonItem *)completeButton{
    self->useCompleteText = YES;
    [_autoCompleteField resignFirstResponder];
    return [super handleActionBarDone:completeButton];
}

- (BOOL)handleActionBarDone:(UIBarButtonItem *)doneButton {
    self->useCompleteText = NO;
    [_autoCompleteField resignFirstResponder];
...
-(void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    if (self->useCompleteText) {
        text = self.lastAutoComplete;
        if ([NSString isNullOrEmpty:text]) {
            text = textField.text;
        }
    }

    _autoCompleteField.text = text;
    _entryElement.textValue = text;
}
